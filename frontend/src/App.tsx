    ...
    <div className="min-h-screen bg-dark-900 font-body">
      ...
      <header className="sticky top-0 z-40 bg-dark-900/90 backdrop-blur-sm border-b border-dark-600">
        <div className="max-w-7xl mx-auto px-8 py-4 flex items-center gap-8">
          ...
          {/* Logo */}
          <button onClick={goHome} className="flex items-center gap-2.5 flex-shrink-0 group">
          ...
          <div className="ml-auto flex-shrink-0">
            <span className="text-xs text-dark-500 font-mono">v0.2.0</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-8 py-10 flex gap-10">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-5">
          ...
          {/* Price chart — shown once a stock is selected */}
          {selectedStock && (
            <PriceChart ticker={selectedStock.ticker} />
          )}
          ...
          {!selectedStock && !loading && (
            <div className="flex flex-col items-center justify-center py-32 gap-6 text-center">
              ...
              <div className="flex flex-col gap-2">
                <p className="text-2xl font-semibold text-dark-100">Track any stock, live.</p>
                <p className="text-base text-dark-400 max-w-sm">
                  Search for a ticker or company name above to see live price charts and the latest news.
                </p>
              </div>
              <div className="flex flex-wrap gap-2 justify-center mt-2">
                {['AAPL', 'TSLA', 'NVDA', 'MSFT', 'META'].map(t => (
                  <span key={t} className="px-3 py-1.5 rounded-lg bg-dark-700 border border-dark-600
                    text-sm font-mono text-dark-300 cursor-default">{t}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {error && (
            <div className="rounded-2xl bg-red-950/30 border border-red-900/40 px-6 py-5 flex items-center gap-4">
              ...
              <button onClick={() => selectedStock && selectStock(selectedStock)}
